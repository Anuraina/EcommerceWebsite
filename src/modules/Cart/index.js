...
const Cart = () => {
  ...
  const handleInc = (id) => {
    const updatedCart = carts.map(item => {
      if(item.id === id) {
        return {
          ...item, //If the id of the current item matches the provided id, the code inside this block will execute. It creates a new object using the spread operator ...item to copy all the properties of the current item, and then it updates the quantity property by incrementing it by 1 (item.quantity + 1).
    ...
    navigate('/cart') //With this setup, you can use the navigate function to change the URL and navigate to a different route within your React application programmatically.
  }
  ...
  const removeProduct = (id) => {
    const updatedCart = carts.filter(item => item.id !== id) //The filter() method is used on the carts array to create a new array that includes only the elements that satisfy the condition inside the filter callback function. In this case, the condition is item.id !== id, which means it will keep all items whose id is not equal to the provided id.
    localStorage.setItem('cart', JSON.stringify(updatedCart))
    navigate('/cart')
  }
